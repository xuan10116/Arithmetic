...
class Node{
    ...
    //左旋转的方法(当右子树的高度减去左子树的高度大于一)
    private void leftRotate(){
        //创建新的节点 newNode，赋予当前节点的值this.value
        Node newNode = new Node(val);
        //把新的节点的左子树 newNode.left 设置成当前节点的左子树this.left
        newNode.left = this.left;
        //把新的节点的右子树 newNode.right 设置成当前节点的右子树的左子树this.right.left
        newNode.right = this.right.left;
        //把当前结点的值this.val替换成它的右子节点的值 this.right.val
        this.val = this.right.val;
        //把当前节点的右子树 this.right 设置成当前节点右子树的右子树this.right.right
        this.right = this.right.right;
        //把当前节点的左子节点this.left 设置成新的节点 newNode
        this.left = newNode;
    }

    //右旋转(当左子树的高度减去右子树的高度大于一)
    private void rightRotate(){
        //创建新节点newNode，赋予当前结点的值this.value
        Node newNode = new Node(this.val);
        //把新结点的右子树newNode.right设置成当前节点的右子树this.right
        newNode.right = this.right;
        //把新结点的左子树newNode.left 设置为当前节点的左子树的右子树 this.left.right
        newNode.left = this.left.right;
        //把当前节点的值this.val 换为左子节点的值 this.left.val
        this.val = this.left.val;
        //把当前节点的左子树 this.left 设置成左子树的左子树 this.left.left
        this.left = this.left.left;
        //把当前节点的右子树 this.right 设置为新节点 newNode
        this.right = newNode;
    }
    ...
    public void add(Node node){
        ...
        //每次添加完一个节点后，判断左右子节点的高度差
        //1,rightHeight() - leftHeight() > 1,需要进行左旋转
        if (rightHeight()-leftHeight() > 1){
            //如果它的右子树的左子树 this.right.left 高度大于它的右子树的右子树this.right.right 的高度
            if(right != null && this.right.leftHeight() > this.right.rightHeight()){
                //先对右节点this.right进行右旋转
                right.rightRotate();
                //然后再对当前节点进行左旋转
                leftRotate();
            }else{
            ...
            return;// rightHeight() - leftHeight() > 1 情况，那么下一个if (leftHeight()-rightHeight() > 1)必定不会指定
        }
        //2,leftHeight() - rightHeight() > 1,需要进行右旋转
        if (leftHeight()-rightHeight() > 1){
            //（当左子节点的右子树(this.left.right)高度大于它的左子树(this.left.left)的高度）
